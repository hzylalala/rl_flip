...
class Go2_BackFlip(BaseTask):
    ...
    def _update_phase(self):
        contact = torch.any(self.contact_forces[:, self.feet_indices, 2] > 1., dim=1)
        height  = self.root_states[:, 2]
        euler_y = self.base_euler_xyz[:, 1]

        # 0 -> 1: 离地
        to_takeoff = (self.phase == 0) & (~contact) & (height > 0.35)
        self.phase[to_takeoff] = 1

        # 1 -> 2: 开始翻转（只要角度过 90° 就认为进入翻转）
        to_flip = (self.phase == 1) & (torch.abs(euler_y) > 0.8)
        self.phase[to_flip] = 2
        self.flip_angle_start[to_flip] = euler_y[to_flip]

        # 2 -> 3: 重新触地
        to_land = (self.phase == 2) & contact
        self.phase[to_land] = 3
        self.landing_timer[to_land] = 0.

        # 3 -> 0: episode 结束或 reset
        #self.landing_timer += self.dt
        self.landing_timer[self.phase == 3] += self.dt
    def post_physics_step(self):
        ...
        # if self.cfg.domain_rand.push_towards_goal and torch.any(env_ids):
        #     #elf._push_robots_desired(env_ids)
    ...
    def reset_idx(self, env_ids):
        ...
        self.landing_timer[env_ids] = 0.
        self.flip_angle_start[env_ids] = 0.
    ...
    def _init_buffers(self):
        ...
        # 在 _init_buffers() 末尾追加
        self.phase = torch.zeros(self.num_envs, dtype=torch.uint8, device=self.device)
        # 0: crouch  1: takeoff  2: flip  3: landing
        self.flip_angle_start = torch.zeros(self.num_envs, device=self.device)
        self.landing_timer    = torch.zeros(self.num_envs, device=self.device)
