...
class Go2_BackFlip(BaseTask):
    ...
    def _reward_before_setting(self):
        ...
        rew = torch.exp(-torch.sum(torch.abs(self.dof_pos - self.lie_joint_pos), dim=1) / 4) * (self.commands[:, 2] == 0)
        return rew
    ...
    def _reward_symmetric_joints(self):
        ...
        # return err
        # 奖励左右对称 (FL vs FR, RL vs RR)
        dof = self.dof_pos.clone().view(self.num_envs, 4, int(self.num_dof/4)) # 0=FL, 1=RL, 2=FR, 3=RR
        # 翻转右侧hip关节符号 (abduction/adduction)
        dof[:,2,0] *= -1 
        dof[:,3,0] *= -1
        
        # 比较 FL vs FR
        err_front = torch.sum(torch.abs(dof[:,0,:] - dof[:,2,:]), axis=1)
        # 比较 RL vs RR
        err_rear = torch.sum(torch.abs(dof[:,1,:] - dof[:,3,:]), axis=1)
